...
public class echoAR : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        try
        {
            ...
            StartCoroutine(QueryDatabase(serverURL+"&entry="+eid));
        }
    ...
    IEnumerator InstantiateModel(Entry entry, List<string> filenames, ImportOptions importOptions)
    {
        ...
        // Set shader
        string shader = null;
        if (entry.getAdditionalData() != null) entry.getAdditionalData().TryGetValue("glTF/PbrMetallicRoughnessDouble", out shader);
